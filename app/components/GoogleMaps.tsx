  ...
  useEffect(() => {
    const loadGoogleMapsScript = () => {
      if (typeof window !== "undefined" && !window.google?.maps) {
        if (!document.querySelector("#google-maps-script")) { // Check if script is already added
          const script = document.createElement("script");
          script.id = "google-maps-script"; // Give an ID to the script
          script.src = `https://maps.gomaps.pro/maps/api/js?key=${process.env.NEXT_PUBLIC_GOMAPS_KEY}&libraries=places&callback=initMap`;
          script.async = true;
          script.defer = true;
